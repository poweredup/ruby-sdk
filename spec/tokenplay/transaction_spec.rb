...
module TokenPlay
  RSpec.describe Transaction do
    ...
    describe '.all' do
      it 'retrieves all the transactions paginated' do
        VCR.use_cassette('transaction/all/paginated') do
          transactions = TokenPlay::Transaction.all(client: client)

          expect(transactions).to be_kind_of TokenPlay::List
          expect(transactions.data.count).to eq 10

          pagination = transactions.pagination
          expect(pagination).to be_kind_of TokenPlay::Pagination
          ...
          expect(transaction).to be_kind_of TokenPlay::Transaction

          expect(transaction.from).to be_kind_of TokenPlay::TransactionSource
          expect(transaction.to).to be_kind_of TokenPlay::TransactionSource
          expect(transaction.exchange).to be_kind_of TokenPlay::Exchange
      ...
      it 'retrieves all the transactions' do
        VCR.use_cassette('transaction/all/custom') do
          transactions = TokenPlay::Transaction.all(
            params: {
              page: 2,
              per_page: 2,
              sort_by: 'created_at',
          ...
          expect(transactions).to be_kind_of TokenPlay::List
          expect(transactions.data.first).to be_kind_of TokenPlay::Transaction
    ...
    describe '.all_for_user' do
      it 'retrieves all the transactions paginated' do
        VCR.use_cassette('transaction/all_for_user/paginated') do
          transactions = TokenPlay::Transaction.all_for_user(
            provider_user_id: ENV['PROVIDER_USER_ID'],
            client: client
          )
